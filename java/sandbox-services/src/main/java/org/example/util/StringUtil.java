...
public class StringUtil {
  ...
  public static String reverse(String input) {
    ...
    logger.trace(
        "Initialized new StringBuilder to collect characters in reverse. "
  ...
  public static boolean isPalindrome(String input) {
    ...
    logger.trace(
        "Created rest sub-array from {} up to but not including index {}", restSubArrayStart, size);

    logger.info(
        "Broken down array to first and rest... Skipping mid if not even\nFirst: {}\nRest: {}",
    ...
    for (int i = 0; i <= mid; i++) {
      logger.trace(
          "Trying to find inequality of first[{}] != rest[{}] ({} != {})?",
          i,
          mid - i,
          first[i],
          rest[mid - i]);
      if (first[i] != rest[mid - i]) {
        logger.info(
            "Compared elements {} and {} are not equal. This is not a palindrom. Returning false...",
            first[i],
            rest[mid - i]);
        return false;
      }
    ...
    logger.info(
        "This is a palindrome as no character pair broke equality pattern. Returning true...");
    return true;
  }
}
